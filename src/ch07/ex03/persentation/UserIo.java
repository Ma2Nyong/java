...
public class UserIo {
	...
	public void play() {
		...
		Console.info(userService.getUser() + "을 추가했습니다."); // 추가
		
		userName = Console.inStr("수정할 회원명을 입력하세요."); //update
		userService.fixUser(userName);
		Console.info(userService.getUser() + "을 수정했습니다."); //database에서 읽어오기.
		
		userService.delUser();
		Console.info("회원을 삭제했습니다.");//delete
	}
}
